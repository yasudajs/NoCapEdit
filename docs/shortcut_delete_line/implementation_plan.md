...
### 手動検証
- [ ] **行削除動作の確認**:
  - [ ] カーソルがある単一行で `Alt + Shift + K` を押し、現在行が削除されること。
  - [ ] 複数行を選択した状態で `Alt + Shift + K` を押し、選択範囲の行が一括削除されること。
  - [ ] Undo (`Ctrl + Z`) / Redo (`Ctrl + Y`) で削除と復元が正しく連動すること。
- [ ] **旧ショートカット無効化の確認**:
  - [ ] `Ctrl + Shift + K` を押しても行削除が発火しないこと。
- [ ] **ヘルプ画面の確認**:
  - [ ] `F1` キーでショートカット一覧を開き、「行の削除」が `Alt + Shift + K` と表示されていること。
- [ ] **ビルド検証**:
  - [ ] `cargo check` 等を実行し、ビルドエラーが発生しないことを確認。
