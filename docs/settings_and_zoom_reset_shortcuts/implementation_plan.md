...
### 1. 設定画面開閉 (`Ctrl + ,`) の検証
- [x] エディタ編集中に `Ctrl + ,` を押すと、設定ドックがスムーズに開くこと
- [x] 設定ドックが開いている状態で `Ctrl + ,`（または Esc）を押すと、設定ドックが閉じてエディタにフォーカスとカーソル位置が復元されること
- [x] 日本語IME変換中に `Ctrl + ,` を押しても誤動作しないこと

### 2. ズーム・行間リセット (`Ctrl + 0`) の検証
- [x] フォントサイズや行間を拡大・縮小した後、`Ctrl + 0` を押すと即座に **20pt / 行間1.5** にリセットされること
- [-] テンキーの `0`（`Numpad0`）およびメインキーの `0`（`Digit0`）の双方でリセットが機能すること
  → テンキーのないPCで開発しているので現時点でテスト不可
- [x] リセット後、ステータスバー右側の表示（20pt / 1.5x）が正しく更新されること
- [x] アプリ再起動後もリセットされた値（20pt / 1.5x）が維持されること

### 3. ヘルプ画面・ビルド検証
- [x] `F1` キーでヘルプ画面を開き、新設のショートカットが正しく表示されていること
