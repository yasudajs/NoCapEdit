...
- **エクスプローラー右クリックメニュー「NoCapEdit で開く」追加およびバイナリ/巨大ファイル安全ガード対応**
  ...
  - **タブ0件時のエディタグレーアウトと案内表示**: 未起動時からバイナリ等を開いて読み込みが失敗した際、エディタエリアをグレーアウト（入力禁止）とし、エディタ左上（通常の入力開始位置・「＋」ボタン真下）に薄い文字で「＋ を押して新規タブを開いてください」と案内を表示。「＋」ボタンを押すことで通常のエディタ入力状態に安全に復帰可能
