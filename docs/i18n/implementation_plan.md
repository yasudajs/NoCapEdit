...
- [x] `main.js` のハードコード文字列を `t()` 呼び出しに置換する。
- [x] `editor.js` のハードコード文字列を `t()` 呼び出しに置換する。

#### フェーズ3b: `tabs.js`（11項目）
- [x] 「未保存」関連のテンプレート処理を含むハードコード文字列を `t()` 呼び出しに置換する。

#### フェーズ3c: `settings.js`（14項目）
- [x] フォントグループラベル、ヒントテキスト等のハードコード文字列を `t()` 呼び出しに置換する。
- [x] 既存の `t()` 呼び出し（`status_ready_auto` 等）を新キー名に更新する。

#### フェーズ3d: `fileSystem.js`（20項目）
- [x] リトライダイアログのテンプレート処理を含むハードコード文字列を `t()` 呼び出しに置換する。
- [x] 既存の `t()` 呼び出しを新キー名に更新する。

#### フェーズ3e: `sidebar.js`（61項目 → 28 ユニークキー）
- [x] D&D 関連のハードコード文字列を `t()` 呼び出しに置換する。
- [x] コピー/切り取り/貼り付け関連のハードコード文字列を `t()` 呼び出しに置換する。
- [x] ツリー表示関連のハードコード文字列を `t()` 呼び出しに置換する。
- [x] 新規作成・名前変更関連のハードコード文字列を `t()` 呼び出しに置換する。
- [x] 削除関連のハードコード文字列を `t()` 呼び出しに置換する。
- [x] 既存の `t()` フォールバック呼び出し（4箇所）をシンプルな `t()` 呼び出しに統一する。

### フェーズ4: テストとクリーンアップ
- [x] 画面上のすべての日本語表示が正常に行われるか確認する。
- [x] 未翻訳（キー名がそのまま表示される等）がないか確認する。
- [x] 旧互換エイリアスキー（4件）を削除する。
- [x] 未使用の孤立キー（5件）を削除する。
