...
- [ ] **2. フロントエンド修正**
  ...
  - [ ] `src/dist/js/core/fileSystem.js` の `triggerManualSave()` リファクタリング (`saveTabIfDirty` への集約)

- [ ] **3. 動作検証 (手動テスト)**
  - [ ] 連番ファイル (`yyyymmdd_hhmmss_1.nctx`) を開いた/保存した際に、タブ表示名が `yyyy/mm/dd hh:mm:ss-1` と表示されるか
  - [ ] 手動保存モード時に連番ファイルが `[yyyy/mm/dd hh:mm:ss-1]` と表示されるか
  - [ ] 手動保存モードで `Ctrl+S` 実行時、同名ファイル衝突時に連番 (`_1`等) で安全に別名保存されるか
  - [ ] 手動保存モードで保存済みファイルに追記・編集して `Ctrl+S` 実行時、既存ファイルに正常に上書き保存されるか
